...
describe("admin.deleteProduct", () => {
  ...
  it("rejects store settings changes for non-admin users", async () => {
    const caller = appRouter.createCaller(createContext("user"));
    await expect(caller.admin.updateSettings({ bankName: "TBC Bank", iban: "GE00TB0000000000000000", receiverName: "Owner", secondBankName: "Bank of Georgia", secondIban: "GE00BG0000000000000000", secondReceiverName: "Owner", shippingFee: 5, bundlePrices: { 2: 39.9, 3: 49.9, 4: 59.9 }, giftLabels: { stickers: { ka: "სტიკერები", en: "Stickers" }, magnet: { ka: "მაგნიტი", en: "Magnet" }, pin: { ka: "პინი", en: "Pin" } }, giftStickersStock: 100, giftMagnetStock: 100, giftPinStock: 100 })).rejects.toMatchObject({ code: "FORBIDDEN" });
