...
export type StoreSettings = { bankName: string; iban: string; receiverName: string; secondBankName: string; secondIban: string; secondReceiverName: string; shippingFee: number; bundlePrices: BundlePrices; facebookFollowers: number; instagramFollowers: number; tiktokFollowers: number; giftLabels: BundleGiftLabels; giftStickersStock: number; giftMagnetStock: number; giftPinStock: number };
const defaultStoreSettings: StoreSettings = { bankName: "TBC Bank", iban: "", receiverName: "", secondBankName: "საქართველოს ბანკი", secondIban: "", secondReceiverName: "", shippingFee: 5, bundlePrices: defaultBundlePrices, facebookFollowers: 0, instagramFollowers: 0, tiktokFollowers: 0, giftLabels: defaultBundleGiftLabels, giftStickersStock: 100, giftMagnetStock: 100, giftPinStock: 100 };
...
export async function getStoreSettings(): Promise<StoreSettings> {
  ...
  const row = rows[0];
  return row ? { bankName: row.bankName, iban: row.iban, receiverName: row.receiverName, secondBankName: row.secondBankName, secondIban: row.secondIban, secondReceiverName: row.secondReceiverName, shippingFee: Number(row.shippingFee), bundlePrices: { 2: Number(row.bundlePrice2), 3: Number(row.bundlePrice3), 4: Number(row.bundlePrice4) }, facebookFollowers: row.facebookFollowers, instagramFollowers: row.instagramFollowers, tiktokFollowers: row.tiktokFollowers, giftLabels: { stickers: { ka: row.giftStickersNameKa, en: row.giftStickersNameEn, image: row.giftStickersImage ?? undefined, active: Boolean(row.giftStickersActive) }, magnet: { ka: row.giftMagnetNameKa, en: row.giftMagnetNameEn, image: row.giftMagnetImage ?? undefined, active: Boolean(row.giftMagnetActive) }, pin: { ka: row.giftPinNameKa, en: row.giftPinNameEn, image: row.giftPinImage ?? undefined, active: Boolean(row.giftPinActive) } }, giftStickersStock: row.giftStickersStock, giftMagnetStock: row.giftMagnetStock, giftPinStock: row.giftPinStock } : defaultStoreSettings;
}
...
export async function updateStoreSettings(input: StoreSettings) {
  ...
  await db.insert(storeSettings).values({ id: 1, bankName: input.bankName, iban: input.iban, receiverName: input.receiverName, secondBankName: input.secondBankName, secondIban: input.secondIban, secondReceiverName: input.secondReceiverName, shippingFee: input.shippingFee.toFixed(2), bundlePrice2: input.bundlePrices[2].toFixed(2), bundlePrice3: input.bundlePrices[3].toFixed(2), bundlePrice4: input.bundlePrices[4].toFixed(2), facebookFollowers: input.facebookFollowers, instagramFollowers: input.instagramFollowers, tiktokFollowers: input.tiktokFollowers, giftStickersNameKa: input.giftLabels.stickers.ka, giftStickersNameEn: input.giftLabels.stickers.en, giftStickersImage: input.giftLabels.stickers.image || null, giftStickersActive: input.giftLabels.stickers.active === false ? 0 : 1, giftMagnetNameKa: input.giftLabels.magnet.ka, giftMagnetNameEn: input.giftLabels.magnet.en, giftMagnetImage: input.giftLabels.magnet.image || null, giftMagnetActive: input.giftLabels.magnet.active === false ? 0 : 1, giftPinNameKa: input.giftLabels.pin.ka, giftPinNameEn: input.giftLabels.pin.en, giftPinImage: input.giftLabels.pin.image || null, giftPinActive: input.giftLabels.pin.active === false ? 0 : 1, giftStickersStock: input.giftStickersStock, giftMagnetStock: input.giftMagnetStock, giftPinStock: input.giftPinStock }).onDuplicateKeyUpdate({ set: { bankName: input.bankName, iban: input.iban, receiverName: input.receiverName, secondBankName: input.secondBankName, secondIban: input.secondIban, secondReceiverName: input.secondReceiverName, shippingFee: input.shippingFee.toFixed(2), bundlePrice2: input.bundlePrices[2].toFixed(2), bundlePrice3: input.bundlePrices[3].toFixed(2), bundlePrice4: input.bundlePrices[4].toFixed(2), facebookFollowers: input.facebookFollowers, instagramFollowers: input.instagramFollowers, tiktokFollowers: input.tiktokFollowers, giftStickersNameKa: input.giftLabels.stickers.ka, giftStickersNameEn: input.giftLabels.stickers.en, giftStickersImage: input.giftLabels.stickers.image || null, giftStickersActive: input.giftLabels.stickers.active === false ? 0 : 1, giftMagnetNameKa: input.giftLabels.magnet.ka, giftMagnetNameEn: input.giftLabels.magnet.en, giftMagnetImage: input.giftLabels.magnet.image || null, giftMagnetActive: input.giftLabels.magnet.active === false ? 0 : 1, giftPinNameKa: input.giftLabels.pin.ka, giftPinNameEn: input.giftLabels.pin.en, giftPinImage: input.giftLabels.pin.image || null, giftPinActive: input.giftLabels.pin.active === false ? 0 : 1, giftStickersStock: input.giftStickersStock, giftMagnetStock: input.giftMagnetStock, giftPinStock: input.giftPinStock } });
...
export function calculateOrderPricing(items: OrderInputItem[], catalog: Product[], shippingFee = 5, bundlePrices: BundlePrices = defaultBundlePrices, giftLabels: BundleGiftLabels = defaultBundleGiftLabels) {
  ...
  for (const [bundleId, items] of Array.from(bundleGroups.entries())) {
    ...
    if (gift && giftLabels[gift].active === false) throw new Error("The selected gift is unavailable. Please choose another gift.");
