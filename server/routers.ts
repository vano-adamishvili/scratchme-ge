...
export const appRouter = router({
  ...
  admin: router({
    ...
    updateSettings: adminProcedure.input(z.object({ bankName: z.string().min(2).max(100), iban: z.string().min(8).max(64), receiverName: z.string().min(2).max(160), secondBankName: z.string().min(2).max(100), secondIban: z.string().min(8).max(64), secondReceiverName: z.string().min(2).max(160), shippingFee: z.number().nonnegative().max(1000), giftStickersStock: z.number().int().nonnegative().max(1_000_000), giftMagnetStock: z.number().int().nonnegative().max(1_000_000), giftPinStock: z.number().int().nonnegative().max(1_000_000) })).mutation(({ input }) => updateStoreSettings(input)),
