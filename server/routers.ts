...
const orderInput = z.object({
  fullName: z.string().min(2), phone: z.string().min(6), address: z.string().min(4), city: z.string().min(2), notes: z.string().optional(), paymentMethod: z.enum(["bank_transfer", "card"]), bankAccount: z.enum(["tbc", "bog"]), total: z.number().nonnegative(), items: z.array(z.object({ productId: z.number(), quantity: z.number().int().positive(), bundleId: z.string().max(64).optional(), bundleTitle: z.string().max(180).optional(), bundleGift: z.enum(["stickers", "magnet", "pin"]).optional() })).min(1),
});
