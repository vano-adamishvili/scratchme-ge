...
function BundleCartItem({ bundle, index, language, productTitle, onRemove }: { bundle: ResolvedCartBundle; index: number; language: "ka" | "en"; productTitle: (product: { title: string; titleKa: string }) => string; onRemove: () => void }) {
  ...
  return <article className="cart-bundle-item"><div className="cart-bundle-head"><div><span className="eyebrow">{language === "ka" ? `ბანდლი ${index + 1}` : `Bundle ${index + 1}`}</span><h3>{label}</h3></div><div className="cart-bundle-price">{formatPrice(bundle.price)}{bundle.tier >= 3 && <small><PackageCheck size={13} /> {language === "ka" ? "უფასო მიტანა" : "Free delivery"}</small>}</div></div><div className="cart-bundle-members">{bundle.products.map((product) => <div key={product.id} className="cart-bundle-member"><img src={product.image} alt="" /><span>{productTitle(product)}</span></div>)}</div>{bundle.tier === 4 && <div className="bundle-gift-summary">{language === "ka" ? "საჩუქარი:" : "Gift:"} {bundle.gift === "stickers" ? (language === "ka" ? "რენდომ 10 სტიკერი" : "Random set of 10 stickers") : bundle.gift === "magnet" ? (language === "ka" ? "თემატური მისაკრობი მაგნიტი" : "Themed magnetic sticker") : (language === "ka" ? "თემატური დასამაგრებელი პინი" : "Themed pin")}</div>}<button className="remove-btn" onClick={onRemove}><Trash2 size={11} /> {language === "ka" ? "მთელი ბანდლის წაშლა" : "Remove bundle"}</button></article>;
}
...
function BundleProgress({ count, freeDelivery, unlockKey }: { count: number; freeDelivery: boolean; unlockKey?: string }) {
  ...
  const message = freeDelivery ? "უფასო მიტანა გახსნილია!" : count >= 4 ? "4-პოსტერიანი ბანდლი გაძლევს უფასო მიტანას" : count === 3 ? "3-პოსტერიანი ბანდლი გაძლევს უფასო მიტანას" : count === 2 ? "ბანდლის შემდეგი დონე: 3 პოსტერი" : "აირჩიე ბანდლი საუკეთესო ფასისთვის";
