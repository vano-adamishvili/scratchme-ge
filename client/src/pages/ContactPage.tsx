...
export default function ContactPage() {
  ...
  const sendMessage = trpc.contact.submit.useMutation({ onSuccess: () => { setForm({ name: "", email: "", message: "" }); showToast(ka ? "შეტყობინება გაიგზავნა" : "Message sent"); }, onError: (error) => showToast(error.message) });
  ...
  const socials = [
    { name: "Facebook", icon: Facebook, href: "https://www.facebook.com/profile.php?id=100069933595746", accent: "social-facebook", followers: settings.data?.facebookFollowers ?? 0 },
    { name: "Instagram", icon: Instagram, href: "https://www.instagram.com/scratchme.ge/", accent: "social-instagram", followers: settings.data?.instagramFollowers ?? 0 },
    { name: "TikTok", icon: Music2, href: "https://www.tiktok.com/@scratchmii", accent: "social-tiktok", followers: settings.data?.tiktokFollowers ?? 0 },
  ];
