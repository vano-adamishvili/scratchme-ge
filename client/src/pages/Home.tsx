...
import { ArrowDown, ArrowRight, BookOpen, Check, Clapperboard, Compass, MapPin, PackageCheck, SlidersHorizontal, Sparkles } from "lucide-react";
...
export default function Home() {
  ...
  return <div className="site-shell"><SiteNav /><main>
    <section className="hero-grid"><div className="hero-copy"><div className="eyebrow">{t("hero.eyebrow")}</div><h1>{t("hero.title")}</h1><p>{t("hero.body")}</p><div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}><a href="#catalog" className="button">{t("hero.shop")} <ArrowRight size={16} /></a><a href="#how-it-works" className="button secondary hero-how-button" aria-label={t("hero.how")}>{t("hero.how")} <ArrowDown size={16} /></a></div><div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 42, color: "var(--muted)", fontSize: 12 }}><MapPin size={14} /> {t("hero.note")}</div></div><div className="hero-art"><div className="hero-sticker">გადაფხიკე / აღმოაჩინე</div><div className="hero-sticker alt">შექმნილია ცნობისმოყვარეებისთვის</div><div className="hero-poster"><div style={{ display: "flex", justifyContent: "space-between", position: "relative", zIndex: 1 }}><span className="eyebrow">scratchme.ge</span><span className="eyebrow">№ 001</span></div><div className="poster-number">100</div><div className="poster-title">ადგილები,<br />სადაც<br />უნდა წახვიდე.</div><div className="poster-footer"><span>საქართველო / ევროპა / მსოფლიო</span><span>გადაფხიკე</span></div></div></div></section>
    ...
    <section className="section how-it-works-section" id="how-it-works" style={{ paddingTop: 34 }}><div className="container"><div className="section-head"><div><div className="eyebrow">03 / {t("trust.eyebrow")}</div><h2>{t("trust.title")}</h2></div><p>{t("trust.intro")}</p></div><div className="trust-grid"><article className="trust-card trust-card-pick" style={{ background: "var(--blue)" }}><div className="trust-card-icon"><Compass size={22} /><span>01</span></div><b>{t("trust.pick")}</b><span>{t("trust.pickBody")}</span></article><article className="trust-card trust-card-discover" style={{ background: "var(--yellow)" }}><div className="trust-card-icon"><Clapperboard size={22} /><span>02</span></div><b>{t("trust.scratch")}</b><span>{t("trust.scratchBody")}</span></article><article className="trust-card trust-card-scratch" style={{ background: "var(--lilac)" }}><div className="trust-card-icon"><BookOpen size={22} /><span>03</span></div><b>{t("trust.goals")}</b><span>{t("trust.goalsBody")}</span><div className="scratch-preview" aria-label="გადასაფხეკი და გადაფხეკილი უჯრები"><span className="scratch-cell revealed">✓</span><span className="scratch-cell revealed">✓</span><span className="scratch-cell">?</span><span className="scratch-cell">?</span><span className="scratch-cell">?</span></div><div className="scratch-demo-wrap"><div className="scratch-demo" role="button" tabIndex={0} aria-label="სატესტო უჯრის გადაფხეკა" onPointerDown={() => setScratchProgress((value) => Math.min(100, value + 18))} onPointerMove={(event) => { if (event.buttons) setScratchProgress((value) => Math.min(100, value + 8)); }} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); setScratchProgress(100); } }} style={{ background: `linear-gradient(135deg, var(--coral) 0 ${scratchProgress}%, var(--cream) ${scratchProgress}% 100%)` }}><span>{scratchProgress >= 100 ? "✓" : (language === "ka" ? "გადაფხიკე აქ" : "Scratch here")}</span></div><button type="button" className="scratch-demo-reset" onClick={() => setScratchProgress(0)}>{language === "ka" ? "თავიდან" : "Reset"}</button></div></article></div><p className="trust-story">{t("trust.story")}</p><Link href="/shop" className="button trust-cta">{t("trust.cta")} <ArrowRight size={16} /></Link><div className="trust-badges"><span><PackageCheck size={16} /> {t("trust.delivery")}</span><span><Sparkles size={16} /> {t("trust.gifting")}</span><span><Check size={16} /> {t("trust.quality")}</span></div></div></section>
