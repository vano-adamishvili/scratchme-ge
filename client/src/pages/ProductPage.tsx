...
import { ArrowLeft, ArrowRight, Check, ChevronLeft, ChevronRight, Truck } from "lucide-react";
import { Link, useLocation, useRoute } from "wouter";
import { Footer, SiteNav } from "@/components/storefront";
import { useI18n } from "@/lib/i18n";
import { formatPrice, getProductGallery } from "@shared/catalog";
...
export default function ProductPage() {
  ...
  const move = (step: number) => setSelectedImage((current) => (current + step + gallery.length) % gallery.length);

  return <div className="site-shell"><SiteNav /><main className="container"><div style={{ paddingTop: 28 }}><Link href="/shop" className="nav-link"><ArrowLeft size={14} style={{ verticalAlign: "-2px" }} /> {t("product.back")}</Link></div><section className="product-detail"><div><div className="detail-image gallery-main"><img src={gallery[selectedImage].url} alt={`${productTitle(product)} — ${language === "ka" ? gallery[selectedImage].labelKa : gallery[selectedImage].labelEn}`} /><div className="detail-label">{language === "ka" ? gallery[selectedImage].labelKa : gallery[selectedImage].labelEn}</div>{gallery.length > 1 && <><button className="gallery-arrow prev" onClick={() => move(-1)} aria-label="Previous image"><ChevronLeft /></button><button className="gallery-arrow next" onClick={() => move(1)} aria-label="Next image"><ChevronRight /></button></>}</div><div className="gallery-thumbs">{gallery.map((image, index) => <button key={`${image.url}-${index}`} className={`gallery-thumb ${selectedImage === index ? "active" : ""}`} onClick={() => setSelectedImage(index)}><img src={image.url} alt="" /><span>{language === "ka" ? image.labelKa : image.labelEn}</span></button>)}</div><div className="info-strip"><div className="info-box"><Check size={17} /><b>{t("product.gift")}</b><span>{t("trust.gifting")}</span></div><div className="info-box"><Truck size={17} /><b>{t("product.fast")}</b><span>{t("product.fastBody")}</span></div><div className="info-box"><b>01 / 100</b><span>{t("trust.goalsBody")}</span></div></div></div><div className="detail-copy"><div className="eyebrow" style={{ color: product.accent }}>{category}</div><h1>{productTitle(product)}</h1>{product.subtitle && <div className="georgian">{product.subtitle}</div>}<p className="description">{productDescription(product.slug, product.description)}</p>{product.features?.length ? <ul className="product-features">{product.features.map((feature) => <li key={feature}><Check size={14} /> {feature}</li>)}</ul> : null}<div className="detail-price">{formatPrice(product.price)}</div><div className={`stock-pill ${outOfStock ? "out" : ""}`}>{outOfStock ? "მარაგში არ არის" : `მარაგშია · ${product.stock ?? 0}`}</div><div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginTop: 18 }}><button className="button coral" disabled={outOfStock} onClick={() => add(false)}>{t("product.add")} <ArrowRight size={16} /></button><button className="button secondary" disabled={outOfStock} onClick={() => add(true)}>{t("product.buy")}</button></div><div style={{ marginTop: 42, paddingTop: 20, borderTop: "1px solid var(--line)" }}><div className="eyebrow">{t("product.fine")}</div><p style={{ color: "var(--muted)", fontSize: 13, lineHeight: 1.55, maxWidth: 390 }}>{t("product.fineBody")}</p></div></div></section></main><Footer /></div>;
}
